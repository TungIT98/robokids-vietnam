...
const require_fileUtils_node = require('../../fileUtils.node-DF-AY0Td.cjs');
const require_FileToolModifier_utils_neutral = require('../../FileToolModifier.utils.neutral-BdMpiyie.cjs');

//#region src/errors/FileModifierErrors.ts
const FileModifierErrorCodes = { FILE_UPLOAD_FAILED: "FILE_UPLOAD_FAILED" };
var ComposioFileUploadError = class extends require_ComposioError.ComposioError {
...
const hydrateFiles = async (value, schema, ctx) => {
	if (schema?.file_uploadable) {
		...
		require_ComposioError.logger_default.debug(`Uploading file "${value}"`);
...
const downloadS3File = async (value, ctx) => {
	...
	try {
		require_ComposioError.logger_default.debug(`Downloading from S3: ${s3url}`);
		const dl = await require_fileUtils_node.downloadFileFromS3({
			toolSlug: ctx.toolSlug,
			s3Url: s3url,
			mimeType: mimetype ?? "application/octet-stream"
		});
		require_ComposioError.logger_default.debug(`Downloaded → ${dl.filePath}`);
		return {
			uri: dl.filePath,
		...
		require_ComposioError.logger_default.error(`Download failed: ${s3url}`, { cause: err });
		return {
			uri: "",
