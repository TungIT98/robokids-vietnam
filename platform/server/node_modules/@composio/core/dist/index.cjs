...
const UpdateConnectedAccountParamsSchema = zod_v3.z.object({
	alias: zod_v3.z.string().optional(),
	connection: zod_v3.z.object({ state: zod_v3.z.object({
		authScheme: zod_v3.z.enum([
			"BEARER_TOKEN",
			"API_KEY",
			"BASIC",
			"BASIC_WITH_JWT",
...
var CustomTools = class {
	...
	customToolsRegistry;
	constructor(client) {
		if (!client) throw new require_ComposioError.ComposioError("ComposioClient is required");
		this.client = client;
	...
	*/
	async getCustomToolBySlug(slug) {
		try {
			...
			require_ComposioError.logger_default.error(`Error getting custom tool: ${error}`);
			return;
		}
	}
	/**
	...
	*/
	async executeCustomTool(slug, body) {
		...
		if (toolkitSlug && toolkitSlug !== "custom" && body.userId) {
			...
			require_ComposioError.logger_default.debug(`[CustomTool] Connected account for ${toolkitSlug} found for user ${body.userId}`, JSON.stringify(connectedAccount, null, 2));
...
var Tools = class {
	...
	*/
	async getRawComposioTools(query, options) {
		...
		require_ComposioError.logger_default.debug(`Fetching tools with filters: ${JSON.stringify(filters, null, 2)}`);
	...
	*/
	async getRawComposioToolBySlug(slug, options) {
		...
		if (customTool) {
			require_ComposioError.logger_default.debug(`Found ${slug} to be a custom tool`, JSON.stringify(customTool, null, 2));
			return customTool;
		} else require_ComposioError.logger_default.debug(`Tool ${slug} is not a custom tool. Fetching from Composio API`);
		let tool;
		try {
	...
	*/
	async executeMetaTool(toolSlug, body, modifiers) {
		...
		if (!executeMetaParams.success) throw new require_CustomTool.ValidationError("Invalid tool execute meta parameters", { cause: executeMetaParams.error });
...
var ConnectedAccounts = class {
	...
	*/
	async initiate(userId, authConfigId, options) {
		...
		else if (connectedAccount.items.length > 0) require_ComposioError.logger_default.warn(`[Warn:AllowMultiple] Multiple connected accounts found for user ${userId} in auth config ${authConfigId}`);
...
var Toolkits = class {
	...
	*/
	...
	async getAuthConfigFields(toolkitSlug, authScheme, authConfigType, requiredOnly) {
		...
		if (toolkit.authConfigDetails.length > 1 && !authScheme) require_ComposioError.logger_default.warn(`Multiple auth configs found for ${toolkitSlug}, please specify the auth scheme to get details of specific auth scheme. Selecting the first scheme by default.`, { meta: { toolkitSlug } });
...
var InternalService = class {
	...
	*/
	async getSDKRealtimeCredentials() {
		...
		require_ComposioError.logger_default.debug(`[InternalService] SDK realtime credentials: ${JSON.stringify(parsedResponse, null, 2)}`);
...
var PusherService = class {
	...
	*/
	async getPusherClient() {
		if (!this.pusherClient) {
			...
			require_ComposioError.logger_default.debug(`[PusherService] Creating Pusher client for client ID: ${this.clientId} in cluster ${this.pusherCluster}`);
			try {
			...
			channel.bind("chunked-" + event, (data) => {
				try {
					...
					if (ev.receivedFinal && ev.chunks.length === Object.keys(ev.chunks).length) try {
						...
						require_ComposioError.logger_default.error("Failed to parse chunked data:", errorMessage);
					} finally {
						delete events[typedData.id];
					}
				} catch (error) {
					const errorMessage = error instanceof Error ? error.message : String(error);
					require_ComposioError.logger_default.error("Error processing chunked trigger data:", errorMessage);
					if (data && typeof data === "object" && "id" in data) delete events[data.id];
				}
			});
		} catch (error) {
			const errorMessage = error instanceof Error ? error.message : String(error);
			require_ComposioError.logger_default.error("Failed to bind chunked events:", error);
	...
	*/
	async subscribe(fn) {
		try {
			require_ComposioError.logger_default.debug(`[PusherService] Subscribing to channel: ${this.pusherChannel}`);
			...
			const safeCallback = (data) => {
				try {
					...
					const errorMessage = error instanceof Error ? error.message : String(error);
					require_ComposioError.logger_default.error("❌ Error in trigger callback:", errorMessage);
				}
			};
			this.bindWithChunking(channel, "trigger_to_client", safeCallback);
			require_ComposioError.logger_default.info(`✅ Subscribed to triggers. You should start receiving events now.`);
		} catch (error) {
	...
	*/
	async unsubscribe() {
		try {
			require_ComposioError.logger_default.debug(`[PusherService] Unsubscribing from channel: ${this.pusherChannel}`);
			await (await this.getPusherClient()).unsubscribe(this.pusherChannel);
			require_ComposioError.logger_default.info(`✅ Unsubscribed from triggers.`);
		} catch (error) {
...
var Triggers = class {
	...
	async create(userId, slug, body) {
		...
		try {
			...
			if (!connectedAccountId) {
				...
				require_ComposioError.logger_default.warn(`[Warn] Multiple connected accounts found for user ${userId}, using the first one. Pass connectedAccountId to select a specific account.`);
			}
		} catch (error) {
	...
	*/
	shouldSendTriggerAfterFilters(filters, data) {
		if (filters.toolkits?.length && !filters.toolkits.map((toolkit) => toolkit.toLowerCase()).includes(data.toolkitSlug.toLowerCase())) {
			require_ComposioError.logger_default.debug("Trigger does not match toolkits filter", JSON.stringify(filters.toolkits, null, 2));
			return false;
		}
		if (filters.triggerId && filters.triggerId !== data.id) {
			require_ComposioError.logger_default.debug("Trigger does not match triggerId filter", JSON.stringify(filters.triggerId, null, 2));
			return false;
		}
		if (filters.connectedAccountId && filters.connectedAccountId !== data.metadata.connectedAccount.id) {
			require_ComposioError.logger_default.debug("Trigger does not match connectedAccountId filter", JSON.stringify(filters.connectedAccountId, null, 2));
			return false;
		}
		if (filters.triggerSlug?.length && !filters.triggerSlug.map((triggerSlug) => triggerSlug.toLowerCase()).includes(data.triggerSlug.toLowerCase())) {
			require_ComposioError.logger_default.debug("Trigger does not match triggerSlug filter", JSON.stringify(filters.triggerSlug, null, 2));
			return false;
		}
		if (filters.triggerData && filters.triggerData !== data.metadata.triggerData) {
			require_ComposioError.logger_default.debug("Trigger does not match triggerData filter", JSON.stringify(filters.triggerData, null, 2));
			return false;
		}
		if (filters.userId && filters.userId !== data.metadata.connectedAccount.userId) {
			require_ComposioError.logger_default.debug("Trigger does not match userId filter", JSON.stringify(filters.userId, null, 2));
			return false;
		}
		require_ComposioError.logger_default.debug("Trigger matches all filters", JSON.stringify(filters, null, 2));
		return true;
	}
	/**
	...
	*/
	async subscribe(fn, filters = {}) {
		...
		require_ComposioError.logger_default.debug("🔄 Subscribing to triggers with filters: ", JSON.stringify(filters, null, 2));
		await this.pusherService.subscribe((_data) => {
			require_ComposioError.logger_default.debug("Received raw trigger data", JSON.stringify(_data, null, 2));
			const parsedData = this.parsePusherPayload(_data);
			if (this.shouldSendTriggerAfterFilters(parsedFilters.data, parsedData)) try {
				fn(parsedData);
			} catch (error) {
				require_ComposioError.logger_default.error("❌ Error in trigger callback:", error);
			}
			else require_ComposioError.logger_default.debug("Trigger does not match filters", JSON.stringify(parsedFilters.data, null, 2));
		});
	}
	/**
	...
	*/
	parsePusherPayload(data) {
		...
		if (versionedResult.ok) {
			require_ComposioError.logger_default.debug(`Parsed Pusher payload as ${versionedResult.version} format`);
			return versionedResult.normalizedPayload;
		}
		const legacyData = data;
		if (legacyData.metadata?.nanoId && legacyData.appName) {
			require_ComposioError.logger_default.debug("Parsed Pusher payload as legacy TriggerData format");
			return transformIncomingTriggerPayload(legacyData);
		}
		require_ComposioError.logger_default.warn("Unknown Pusher payload format. Payload keys: " + Object.keys(data).join(", "));
...
const userDataPath = () => {
	try {
		...
		return _platform.platform.joinPath(homeDir, require_ComposioError.COMPOSIO_DIR, require_ComposioError.USER_DATA_FILE_NAME);
	} catch (_error) {
		require_ComposioError.logger_default.debug("Environment", `Unable to get user data path`);
		return null;
	}
};
/**
...
const getUserDataJson = () => {
	try {
		...
		require_ComposioError.logger_default.debug("Environment", "No user data file found");
		return {};
	}
};
function getSDKConfig(baseUrl, apiKey) {
	const { api_key: apiKeyFromUserConfig, base_url: baseURLFromUserConfig } = getUserDataJson();
	const baseURLParsed = baseUrl || require_ComposioError.getEnvVariable("COMPOSIO_BASE_URL") || baseURLFromUserConfig || require_ComposioError.DEFAULT_BASE_URL;
	const apiKeyParsed = apiKey || require_ComposioError.getEnvVariable("COMPOSIO_API_KEY") || apiKeyFromUserConfig || "";
	if (!apiKeyParsed) require_ComposioError.ComposioError.handleAndThrow(new require_CustomTool.ComposioNoAPIKeyError());
	require_ComposioError.logger_default.debug("Environment", `API Key: ${apiKeyParsed}`);
	require_ComposioError.logger_default.debug("Environment", `Base URL: ${baseURLParsed}`);
	return {
...
function getToolkitVersionsFromEnv(defaultVersions) {
	...
	const envPrefixedVersions = require_ComposioError.getEnvsWithPrefix(`COMPOSIO_TOOLKIT_VERSION_`);
...
async function checkForLatestVersionFromNPM(currentVersion) {
	try {
		...
		if (semver.default.gt(latestVersion, currentVersionFromPackageJson) && !require_ComposioError.IS_DEVELOPMENT_OR_CI) require_ComposioError.logger_default.info(`🚀 Upgrade available! Your composio-core version (${currentVersionFromPackageJson}) is behind. Latest version: ${latestVersion}.`);
	} catch (_error) {}
}
	...
	*/
	async handleAssistantMessage(userId, run, options, modifiers) {
		...
		return await Promise.all(tool_calls.map(async (tool_call) => {
			require_ComposioError.logger_default.debug(`Executing tool call: ${tool_call.id}`);
			const tool_response = await this.executeToolCall(userId, tool_call, options, modifiers);
			require_ComposioError.logger_default.debug(`Tool call ${tool_call.id} executed with response: ${tool_response}`);
			return {
...
const ToolRouterSessionFilesMountListOptionsSchema = zod_v3.default.object({
	...
	mountId: zod_v3.default.string().default(require_ComposioError.DEFAULT_TOOL_ROUTER_SESSION_FILES_MOUNT_ID),
...
const ToolRouterSessionFilesMountDownloadOptionsSchema = zod_v3.default.object({ mountId: zod_v3.default.string().default(require_ComposioError.DEFAULT_TOOL_ROUTER_SESSION_FILES_MOUNT_ID) });
const ToolRouterSessionFilesMountDeleteOptionsSchema = zod_v3.default.object({ mountId: zod_v3.default.string().default(require_ComposioError.DEFAULT_TOOL_ROUTER_SESSION_FILES_MOUNT_ID) });
...
var RemoteFile = class RemoteFile {
	...
	*/
	async save(path) {
		...
		const savePath = path ?? _platform.platform.joinPath(homeDir, require_ComposioError.COMPOSIO_DIR, require_ComposioError.TEMP_FILES_DIRECTORY_NAME, this.filename);
		const dir = path != null ? getParentDir(savePath) : _platform.platform.joinPath(homeDir, require_ComposioError.COMPOSIO_DIR, require_ComposioError.TEMP_FILES_DIRECTORY_NAME);
...
var SessionContextImpl = class {
	userId;
	constructor(client, userId, sessionId, customToolsMap) {
		this.client = client;
		this.sessionId = sessionId;
...
var ToolRouterSession = class {
	...
	sessionContext;
	constructor(client, config, sessionId, mcp, experimentalOverrides, customToolsMap, userId) {
		this.client = client;
		this.config = config;
		this.customToolsMap = customToolsMap;
		this.userId = userId;
		...
		if (customToolsMap && userId) this.sessionContext = new SessionContextImpl(client, userId, sessionId, customToolsMap);
	...
	*/
	async tools(modifiers) {
		...
		const tools = await ToolsModel.getRawToolRouterMetaTools(this.sessionId, modifiers?.modifySchema ? { modifySchema: modifiers.modifySchema } : void 0);
		if (this.hasCustomTools()) {
			const routingExecuteFn = async (toolSlug, input) => {
				if (toolSlug === COMPOSIO_MULTI_EXECUTE_TOOL) return this.routeMultiExecute(input, ToolsModel, modifiers);
				return ToolsModel.executeMetaTool(toolSlug, {
			...
			return this.config.provider.wrapTools(tools, routingExecuteFn);
		}
		return ToolsModel.wrapToolsForToolRouter(this.sessionId, tools, modifiers);
	}
	/**
	...
	customToolkits() {
		...
		return toolkits.map((tk) => ({
			...
			tools: tk.tools.map((tool) => {
				return {
					slug: this.customToolsMap.byOriginalSlug.get(tool.slug.toUpperCase())?.finalSlug ?? tool.slug,
					name: tool.name,
					description: tool.description,
					toolkit: tk.slug,
	...
	*/
	async toolkits(options) {
		...
		return {
			...
			nextCursor: result.next_cursor ?? void 0,
	...
	*/
	async search(params) {
		const transformed = transformSearchResponse(await this.client.toolRouter.session.search(this.sessionId, {
	...
	*/
	async execute(toolSlug, arguments_) {
		...
		const transformed = transformExecuteResponse(await this.client.toolRouter.session.execute(this.sessionId, {
			tool_slug: toolSlug,
			arguments: arguments_ ?? {}
		}));
	...
	*/
	async routeMultiExecute(input, ToolsModel, modifiers) {
		const toolItems = input.tools;
		if (!Array.isArray(toolItems) || toolItems.length === 0) return ToolsModel.executeMetaTool(COMPOSIO_MULTI_EXECUTE_TOOL, {
			sessionId: this.sessionId,
			arguments: input
		}, modifiers);
		...
		for (let i = 0; i < parsed.length; i++) {
			...
			else remoteIndices.push(i);
		}
		if (localItems.length === 0) return ToolsModel.executeMetaTool(COMPOSIO_MULTI_EXECUTE_TOOL, {
			sessionId: this.sessionId,
			arguments: input
		}, modifiers);
		...
		if (remoteIndices.length > 0) {
			...
			remotePromise = ToolsModel.executeMetaTool(COMPOSIO_MULTI_EXECUTE_TOOL, {
				sessionId: this.sessionId,
...
var ToolRouter = class {
	...
	*/
	async create(userId, config) {
		...
		if (customTools?.length) experimentalPayload.custom_tools = require_CustomTool.serializeCustomTools(customTools);
		if (customToolkits?.length) experimentalPayload.custom_toolkits = require_CustomTool.serializeCustomToolkits(customToolkits);
		...
		return new ToolRouterSession(this.client, this.config, session.session_id, this.createMCPServerConfig(session.mcp), { assistivePrompt }, customToolsMap, userId);
	}
	/**
	...
	*/
	async use(id) {
		...
		return new ToolRouterSession(this.client, this.config, session.session_id, this.createMCPServerConfig(session.mcp));
	}
};
...
var Composio = class Composio {
	...
	*/
	constructor(config) {
		...
		if (require_ComposioError.IS_DEVELOPMENT_OR_CI) require_ComposioError.logger_default.debug(`Initializing Composio w API Key: [REDACTED] and baseURL: ${baseURLParsed}`);
		/**
		...
		*/
		this.client = new _composio_client.default({
			...
			logLevel: require_ComposioError.COMPOSIO_LOG_LEVEL
		});
...
exports.ComposioError = require_ComposioError.ComposioError;
...
Object.defineProperty(exports, 'constants', {
  enumerable: true,
  get: function () {
    return require_ComposioError.constants_exports;
  }
});
...
exports.logger = require_ComposioError.logger_default;
