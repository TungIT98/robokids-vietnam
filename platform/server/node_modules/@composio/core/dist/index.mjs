...
const UpdateConnectedAccountParamsSchema = z$1.object({
	alias: z$1.string().optional(),
	connection: z$1.object({ state: z$1.object({
		authScheme: z$1.enum([
			"BEARER_TOKEN",
			"API_KEY",
			"BASIC",
...
const ToolRouterCreateSessionConfigSchema = z.object({
	...
	workbench: z.object({
		enable: z.boolean().default(true).describe("Whether to enable the workbench entirely. Defaults to true. When set to false, no code execution tools (COMPOSIO_REMOTE_WORKBENCH, COMPOSIO_REMOTE_BASH_TOOL) are available in the session."),
...
var RemoteFile = class RemoteFile {
	...
	*/
	async save(path) {
		...
		const savePath = path ?? platform.joinPath(homeDir, COMPOSIO_DIR, TEMP_FILES_DIRECTORY_NAME, this.filename);
		...
		if (typeof input === "string") {
			if (input.startsWith("http://") || input.startsWith("https://")) {
				...
				if (!filename$1 || !filename$1.includes(".")) {
					...
					filename$1 = `${getRandomShortId()}.${extension}`;
				}
				return {
...
var ToolRouter = class {
	...
	*/
	async create(userId, config) {
		...
		if (customTools?.length) experimentalPayload.custom_tools = serializeCustomTools(customTools);
		if (customToolkits?.length) experimentalPayload.custom_toolkits = serializeCustomToolkits(customToolkits);
