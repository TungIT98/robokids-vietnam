...
const uploadFileToS3 = async (fileName, content, toolSlug, toolkitSlug, mimeType, client) => {
	...
	require_ComposioError.logger_default.debug(`Uploading ${key} file to S3: ${key}`);
...
const getFileDataAfterUploadingToS3 = async (file, { toolSlug, toolkitSlug, client }) => {
	...
	require_ComposioError.logger_default.debug(`Uploading file to S3...`);
	...
	require_ComposioError.logger_default.debug(`Done! File uploaded to S3: ${s3key}`, JSON.stringify(fileData, null, 2));
	return {
...
const getComposioDir = (createDirIfNotExists = false) => {
	try {
		...
		const composioDir = _platform.platform.joinPath(homeDir, require_ComposioError.COMPOSIO_DIR);
...
const getComposioTempFilesDir = (createDirIfNotExists = false) => {
	try {
		const homeDir = _platform.platform.homedir();
		if (!homeDir) return null;
		const composioFilesDir = _platform.platform.joinPath(homeDir, require_ComposioError.COMPOSIO_DIR, require_ComposioError.TEMP_FILES_DIRECTORY_NAME);
...
const saveFile = (file, content, isTempFile = false) => {
	try {
		if (!_platform.platform.supportsFileSystem) {
			require_ComposioError.logger_default.debug("File system operations are not supported in this runtime environment");
			return null;
		}
		...
		require_ComposioError.logger_default.info(`Saving file to: ${filePath}`);
		...
		require_ComposioError.logger_default.debug(`Error saving file: ${_error}`);
		return null;
	}
};
